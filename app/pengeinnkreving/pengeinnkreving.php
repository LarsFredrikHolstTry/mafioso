...
if (player_in_bunker($_SESSION['ID'], $pdo)) {
    ...
    if (isset($_GET['nullcd'])) {
        echo feedback("Du nullstilte ventetiden for " . $resetCDPrice . " poeng!", "success");
    }

    if (isset($_POST['nullstillCD'])) {
        if ($resetCDPrice > AS_session_row($_SESSION['ID'], 'AS_points', $pdo)) {
            echo feedback("Du har ikke nok poeng til å nullstille ventetiden", "fail");
        } else {
            $sql = "UPDATE cooldown SET CD_event = ? WHERE CD_acc_id = ?";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([0, $_SESSION['ID']]);

            take_poeng($_SESSION['ID'], $resetCDPrice, $pdo);
            header("Location: ?side=pengeinnkreving&nullcd");
        }
    }
            ...
            <div class="content">
                <?php if ($isMemberOfAGroup) {
                    ...
                    if (event_ready($_SESSION['ID'], $pdo)) {
                        ...
                        <form method="post">
                            <input type="submit" style="width: 100%;" value="Nullstill ventetiden for 75 poeng" name="nullstillCD" />
                        </form>
